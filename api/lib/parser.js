...
const parseGPX = (buffer) => {
  const data = XMLParser.parse(buffer.toString('utf8'), { attributeNamePrefix : "@", ignoreAttributes: false })
  const GPX = data.gpx
  ...
  const points = tracks.reduce((points, track) => {
    ...
    const flatten = segments.reduce((points, segment) => points.push(...segment.trkpt.map(pt => ({
